...
| Wave 2 | 引入 execution slot 骨架 | `execution_slot_id`、slot 状态、slot 与 task-run 关联 | Wave 1 | 未开始 |
| Wave 3 | 引入 workspace lease | slot 绑定独立工作区实例；共享主工作区不再直接承载执行现场 | Wave 2 | 未开始 |
...
| Governance G1 | 原子共享协调状态 | write intent、冲突策略、追加式协调审计 | Wave 2 | 未开始 |
...
- [ ] 补一份“入口文件重构边界”说明，明确哪些逻辑留在 `main.rs`
- [x] 跑状态归一化相关测试

### Wave 2：execution slot 骨架

- [ ] 在数据模型中加入 `execution_slot_id`
- [ ] 明确 task-run 与 slot 的一对多关系
- [ ] 把当前恢复入口从 task 视角扩成 slot 视角
- [ ] 为 slot 增加最小状态字段与审计活动
- [ ] 补 slot 创建、释放、冲突回收测试

### Wave 3：workspace lease

- [ ] 在 slot 上绑定 `workspace_lease_id`
- [ ] 将项目主工作区与执行工作区实例解耦
- [ ] 给 lease 增加 owner、生命周期、回收原因
...
- [ ] 为 task / slot / lease / artifact / decision / budget 引入共享协调写入边界
- [ ] 设计 `write_intent` / `validation` / `commit` 等价数据模型
- [ ] 至少支持 `first-commit-wins` 与 `priority-wins` 两种冲突策略
- [ ] 关键协调写入进入追加式审计链路
- [ ] 补并发冲突、陈旧写入、重复提交回归测试
...
- [ ] 继续清理 `main.rs` 中残留的跨模块共享 helper，例如 `resolve_workspace_for_task` / `record_task_activity` / `task_developer_instructions`
- [ ] 评估 `execution slot` 与 atomic coordination state 的公共数据模型切入点
